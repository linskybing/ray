...
DEFAULT_TOTAL_SAMPLES: Final[int] = 5120
...
def run_distributed_pipeline(
    ...
    # 3. Launch pipelines in parallel
    pipeline_refs = [w.run_pipeline.remote(config=config) for w in workers]
    ...
    summary = {
        ...
        "config": {
            ...
            "total_samples_per_worker": config.total_samples,
        },
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Total samples each worker processes",
    )
    parser.add_argument(
        "--cpu-batch-size",
        type=int,
    ...
    try:
        ...
        print(f"  Samples/worker: {config.total_samples}")
